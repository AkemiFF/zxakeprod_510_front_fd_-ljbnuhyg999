...
const registerUser = async (email: any, password: any) => {
  const response = await fetch("http://127.0.0.1:8000/api/accounts/client/create/", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      ...
      if (errorData && errorData.email) {
        toast.error(errorData.email[0], { autoClose: 5000 });
      }
      if (errorData && errorData.password1) {
        toast.error(errorData.password1[0], { autoClose: 5000 });
      }
    }
  };
